...
@KafkaListener(topics = "${kafka.topic.internal_operation}", groupId = "${spring.kafka.consumer.group-id}")
public class InternalOperationMessageListenerImpl implements TransferListener<Transfer> {
    ...
    public void listenTransfer(Transfer message) {
        ...
        try {
            transferService.internalFundTransfer(transferMapper.toTransfer(message));
