...
@KafkaListener(topics = "${kafka.topic.credit_operation}", groupId = "${spring.kafka.consumer.group-id}")
public class CreditOperationMessageListenerImpl implements TransferListener<Transfer> {
    ...
    public void listenTransfer(Transfer message) {
        ...
        try {
            transferService.creditTransfer(transferMapper.toTransfer(message));
