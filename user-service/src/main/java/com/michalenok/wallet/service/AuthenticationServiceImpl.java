...
import com.michalenok.wallet.mapper.VerificationMapper;
...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    private final MessageProducer<Verification> messageProducer;

    @Override
    @Transactional
    public void register(UserRegistrationDto user) {
        userService.create(userMapper.userRegistrationDtoToUserCreateDto(user));
        VerificationEntity verificationEntity = generateVerificationEntity(user.mail());
        verificationRepository.save(verificationEntity);
        sendVerificationMessage(verificationMapper.toVerification(verificationEntity));
        log.info("Successful registration user with mail: {}", user.mail());
    }
    ...
    public void verifyUser(String code, String mail) {
        ...
        log.info("Successful verification user with mail {} and code {}", mail, code);
    }
    ...
    private void isCodeValid(String code, String mail) {
        VerificationEntity verificationCode = verificationRepository.findById(mail)
                .orElseThrow(() ->
                        new VerificationUserException(String.format("There is no code for user with mail %s", mail)));
    ...
    private void isStatusActivated(UserEntity user) {
        if (user.getStatus() != UserStatus.ACTIVATED) {
            log.error("Unsuccessful login user with mail {}. User is not activated", user.getMail());
    ...
    private VerificationEntity generateVerificationEntity(String mail) {
        ...
        log.info("Create verification code {} for user: {}", verificationEntity.getCode(), verificationEntity.getMail());
        return verificationEntity;
    }

    private void sendVerificationMessage(Verification verification) {
        log.info("Send  verification code {}  to user: {}", verification.getCode(), verification.getMail());
        messageProducer.sendMessage(verification);
    }
}
